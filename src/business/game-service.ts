...
export function gameService(dom: Ref<HTMLElement>) {
  ...
  const initChessBoard = () => {
    if (!dom?.value) throw Error("need a root dom ref at function gameService.")
    chessborad = new ChessBoard({ row: GameConfig.row, column: GameConfig.column });
    ...
    dom.value.setAttribute("style", Object.keys(rootElemStyle).reduce<string>((p, k) => p += `${k}:${rootElemStyle[k]};` as string, ""))
  }
  ...
  const initChess = async () => {
    ...
    const defineTotal = GameConfig.layers * GameConfig.quantityPerLayer + GameConfig.quantityRandom;
    // 棋子应该是类型的整数倍，才能消除完
    const shouldTotal = defineTotal + perRemoveTotal - defineTotal % perRemoveTotal;
    ...
    for (let i = 0; i < GameConfig.layers && leftNoPosChesss; i++) {
      let currentChesss = (i === GameConfig.layers - 1) ? leftNoPosChesss : Math.min(leftNoPosChesss, GameConfig.quantityPerLayer);
      ...
      const waitGenChess = allChess.slice(i * GameConfig.quantityPerLayer, i * GameConfig.quantityPerLayer + currentChesss);
      genPosChess(waitGenChess, i, pos.minX, pos.minY, pos.maxX, pos.maxY);
      if (leftNoPosChesss <= 0) break;
    }
    gameState.boardChesses = allChess.slice(0, chessboardTotal)
  }
   ...
   * @param maxY y最大偏移
   */
  const genPosChess = (chesses: Chess[], layer: number, minX: number, minY: number, maxX: number, maxY: number) => {
    ...
    for (let i = 0; i < chesses.length; i ++) {
      ...
      chessborad.fill(x, y, chesses[i]);
      chesses[i].x = x;
      chesses[i].y = y;
      posRecord.add(point);
      genLayerRelation(chesses[i]);
    }
  }
  ...
  const genLayerRelation = (chess: Chess) => {
    ...
    const maxX = Math.min(chess.x + GameConfig.perChessColumn - 1, GameConfig.column - (GameConfig.perChessColumn - 1));
    const maxY = Math.min(chess.y + GameConfig.perChessRow - 1, GameConfig.row - (GameConfig.perChessRow - 1));
    ...
    for (let i = minX; i < maxX; i++) {
      for (let j = minY; j < maxY; j++) {
        ...
        if (relationBlocks!?.length > 0) {
          ...
          // if (maxLevelRelationBlock.layer > chess.layer) {
          //   chess.highers.push(maxLevelRelationBlock);
          //   maxLevelRelationBlock?.lowers.push(chess);
          // } else if (maxLevelRelationBlock.layer < chess.layer){
